...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N): # for each neuron
        for f in range(F): # for each filter/kernel
            for i in range(0, H_out): # for each y activation
                for j in range(0, W_out): # for each x activation
...
def max_pool_forward_naive(x, pool_param):
    ...
    assert (H - pool_height) % stride == 0, '[Sanity Check] [FAIL]: Pooling Layer Failed in Height'
    assert (W - pool_width) % stride == 0,  '[Sanity Check] [FAIL]: Pooling Layer Failed in Width'
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
      for c in range(C):
        for h in range(H_out):
          for w in range(W_out):
            ind = np.argmax(x[n, c, h*stride:h*stride+pool_height, w*stride:w*stride+pool_width])
            ind_1, ind_2 = np.unravel_index(ind, (pool_height, pool_width))
            dx[n, c, h*stride:h*stride+pool_height, w*stride:w*stride+pool_width][ind_1, ind_2] = dout[n,c,h,w]
